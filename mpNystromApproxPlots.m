...
for k=k_loop 
    ...
    for rndseed = 1:rnd_runs
        ...
        if halfOn
            ...
            [Uh, Lambdah, Y] = NystromSketch(A, n, k+l,'h',rndseed,false);
            Uh = double(Uh(:,1:k));
...
text(127,8*1e-2,' k=127','FontSize',80)
text(1077,4*1e-7,' k=1075','HorizontalAlignment','right','FontSize',80)
...
boundfin_d = n^(1/2)*(nud/(1-nud))*normA;
boundfin_s = n^(1/2)*(nus/(1-nus))*normA;
boundfin_h = n^(1/2)*(nuh/(1-nuh))*normA;
boundfin_q43 = n^(1/2)*(nuq43/(1-nuq43))*normA;
boundfin_q52 = n^(1/2)*(nuq52/(1-nuq52))*normA;
